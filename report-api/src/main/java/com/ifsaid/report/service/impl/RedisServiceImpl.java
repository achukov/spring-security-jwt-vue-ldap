...
import com.ifsaid.report.service.IRedisService;
    ...
    public Boolean hasSetValue(String key, Object obj) {
        ...
        while (true) {
            try {
                ...
                log.error("key[" + key + "],obj[" + obj + "] to determine whether the value in the Set has failed, the exception information:" + e.getMessage());
                t++;
            }
            if (t > times) {
                break;
        ...
        log.info("key[" + key + "],obj[" + obj + "Whether it exists, boo:" + boo);
        return boo;
    }
